...
describe('buildLayerTree', () => {
  ...
  it('xdts検出後のアニメーションフォルダ内の_プレフィックスは自動マークしない', () => {
    ...
    const xdts = { tracks: [{ name: 'A', cellNames: [], frames: [] }], version: 5, header: { cut: '1', scene: '1' }, timeTableName: 'タイムライン1', duration: 72, fps: 24 }
...
describe('detectAnimationFoldersByXdts', () => {
  it('xdtsのtrack名でアニメーションフォルダを検出する', () => {
    ...
    // tree[0] = A, tree[1] = B
    const tree = buildLayerTree(psd)
    const xdts: XdtsData = { tracks: [{ name: 'A', cellNames: [], frames: [] }], version: 5, header: { cut: '1', scene: '1' }, timeTableName: 'タイムライン1', duration: 72, fps: 24 }
    detectAnimationFoldersByXdts(tree, xdts)
    expect(tree[0].isAnimationFolder).toBe(true)
  ...
  it('tracks配列にある名前のフォルダはすべてアニメーションフォルダとして検出する', () => {
    ...
    const tree = buildLayerTree(psd)
    const xdts: XdtsData = { tracks: [{ name: 'CAM', cellNames: [], frames: [] }], version: 5, header: { cut: '1', scene: '1' }, timeTableName: 'タイムライン1', duration: 72, fps: 24 }
    detectAnimationFoldersByXdts(tree, xdts)
    expect(tree[0].isAnimationFolder).toBe(true)
  })
