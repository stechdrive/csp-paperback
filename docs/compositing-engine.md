...
### `_`プレフィックスフォルダの工程フォルダ自動判定

`_` で始まるフォルダ（単体出力の対象）のうち、**直接の子にプロジェクト設定テーブル（工程テーブル）登録名のフォルダを持つもの** は、アニメーションフォルダとして自動的に扱う（実装は `promoteAutoMarkedByProcessMatch`）。これは「作画マンの背景原図と、それに対する演出修正（`_e/` 等）を別々の紙として出力したい」というユースケースを、明示的な手動指定なしで満たすための機構である。
...
アニメーションフォルダ特定と `_` 自動出力の優先順位：

```
XDTS検出 > 手動指定 > 工程フォルダ自動判定（子孫アニメフォルダ優先）> 整理用 `_` 親フォルダ抑制 > `_`プレフィックス単体出力マーク
```
...
`_` で始まるフォルダのうち、直下の表示中要素が出力単位だけで構成されるものは、統合画像を作らず整理用コンテナとして扱う。これは `CSP` 上で素材フォルダを束ねるためだけに `_原図/` などを置き、その直下の `_BG/`・`_BOOK/`・XDTS由来アニメーションフォルダだけが実際の必要素材であるケースを想定する。
...
判定は直下だけを見る。直下に通常レイヤーまたは通常フォルダが混じる場合は、親 `_` フォルダ自体の統合画像が必要な可能性があるため抑制しない。非表示の直下要素は出力対象外なので判定から除外する。親フォルダの不透明度・合成モードは、CSP上での表示調整に使われる可能性があるため抑制判定には使わない。
