...
通常のアニメーションフォルダ（XDTS/手動指定）では、直下のセル名は連番（`0001`, `0002` …）、連番+セル名（`0001_セル名`）、セル名（CSP上のレイヤー名）を出力設定で選択できる。一方、自動アニメ化で扱うフォルダ（`_BG`, `_BOOK1` 等）では、**直下のフォルダ名そのものが素材分類の意味情報**（背景／BOOK／セルの役割等）を持つ。
